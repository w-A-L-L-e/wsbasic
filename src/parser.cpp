...
void Parser::Error(const string &s) {
  cerr << "Error: " << s << " at row " << row << " , col " << col << "."
       << endl;
  // exit(1); // better = throw exception here!
  bNoErrors = false;
}

void Parser::Expected(const string &s) { Error("Expected " + s); }
...
TreeNode *Parser::getTree() { return tree; }
...
TreeNode *Parser::Factor() {
  ...
  case tokString:
    n = new TreeNode(stringConstantNode, row, col);
    n->setValue(look.str);
    n->setName("const str");
    Match(tokString);
    break;
  ...
  case tokNumber:
    n = new TreeNode(constantNode, row, col);
    n->setValue(look.val);
    n->setName("number");
    Match(tokNumber);
    break;
  ...
  default:
    Error("Illegal char in expression");
...
bool Parser::isAddOp(token t) {
  return (t.type == '+') || (t.type == '-') || (t.type == tokMod) ||
         (t.type == tokGt) || (t.type == tokGe) || (t.type == tokLt) ||
         (t.type == tokLe) || (t.type == tokEq) || (t.type == tokNe) ||

         (t.type == tokOr) || (t.type == tokGe);
}
...
/*
  <for> ::= tokFor <id>'='<expression> tokTo <expression> (<tokStep>
  <expression>)? ( <statement> | <block> ) for loops with step have 5 children
...
/*
  tokForEach <id> tokIn <expression> (tokSeperated tokBy <expression>)?
  (<statement>|<block>) 4 children default seperator is "\n"
*/
...
TreeNode *Parser::Statement() {
  ...
  case tokId:
    return Other();
    break; // assignment or function call
  ...
  case tokBegin:
    Error("Begin without matching end");
    getToken();
...
// main block added so you don't have to wrap code in begin/end, is more python
// like as well
