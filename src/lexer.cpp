...
int Lexer::getRow() { return row; }

int Lexer::getCol() { return col; }
...
void Lexer::checkKeywords(token &t) {
  if (t.type == tokId) {
    if (t.str == "begin")
      ...
      t.type = tokWrite;
    else if (t.str == "substr")
      t.type = tokSubstr;
    else if (t.str == "foreach")
      t.type = tokForEach;
    else if (t.str == "in")
      t.type = tokIn;
    else if (t.str == "seperated")
      t.type = tokSeperated;
    else if (t.str == "by")
      t.type = tokBy;
  ...
  while (ch != '"' && !in->eof()) {
    ...
    if (ch == '\\') { // escape sequence
      ...
      case '"':
        ...
        break;
      default:
        cerr << "Unrecognized escape char \\" << (char)ch
             << " in stringconstant, skipping!" << endl;
        break;
      }
