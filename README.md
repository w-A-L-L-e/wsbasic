...
Changed syntax slightly. Now whenever you have an if, def, while, for
we always tart a block (so you dont need to add begin). This does
mean you always need an end statement after these. And futhermore for if/else
we added an elsif so you can have short if/elsif/elsif/else blocks
See this in action in the examples/mandelbrot.b
Functions example:
...
The hello world also much shorter because main block does not need
a begin nor an end (the end of file acts as ending of block here):
